...
const RuleForm = ({ rule, onSave, onCancel }) => {
  ...
  useEffect(() => {
    if (isEditing && rule) {
      ...
      setSellerMode(
        Array.isArray(rule.sellerIds) && rule.sellerIds.length > 0 ? 'specific' : 'all',
      );
    } else {
      setFormData({
        name: '',
      ...
      <div className='w-full max-w-3xl bg-white rounded-2xl shadow-2xl border border-gray-200 my-8 animate-scale-in'>
        ...
        <div className='px-6 py-5 space-y-5'>
          ...
          <div>
            ...
            {errors.description && (
              <p
                id='rule-form-description-error'
                className='text-xs text-red-600 mt-1.5 flex items-center gap-1'
              >
                <svg
                  xmlns='http://www.w3.org/2000/svg'
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <input
                ...
                aria-describedby={
                  errors.effectiveDate ? 'rule-form-effective-date-error' : undefined
                }
                aria-invalid={errors.effectiveDate ? 'true' : 'false'}
              />
              {errors.effectiveDate && (
                <p
                  id='rule-form-effective-date-error'
                  className='text-xs text-red-600 mt-1.5 flex items-center gap-1'
                >
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
