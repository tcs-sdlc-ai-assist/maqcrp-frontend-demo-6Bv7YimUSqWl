...
const COMPONENT_NAME = 'FilteredRemedyList';
...
const FilteredRemedyList = ({ filters: externalFilters }) => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { filterRemedies } = useRemedies();
  ...
  useEffect(() => {
    ...
    if (externalFilters && typeof externalFilters === 'object') {
      if (externalFilters.status && typeof externalFilters.status === 'string') {
        initialFilters.status = externalFilters.status;
      }
      if (externalFilters.priority && typeof externalFilters.priority === 'string') {
        initialFilters.priority = externalFilters.priority;
      }
      if (externalFilters.remedyType && typeof externalFilters.remedyType === 'string') {
        initialFilters.remedyType = externalFilters.remedyType;
      }
      if (externalFilters.sourceType && typeof externalFilters.sourceType === 'string') {
        initialFilters.sourceType = externalFilters.sourceType;
      }
      if (externalFilters.slaStatus && typeof externalFilters.slaStatus === 'string') {
        initialFilters.slaStatus = externalFilters.slaStatus;
      }
      if (externalFilters.counterpartyId && typeof externalFilters.counterpartyId === 'string') {
        initialFilters.search = externalFilters.counterpartyId;
      }
    }
    ...
    if (urlPriority && typeof urlPriority === 'string') {
      initialFilters.priority = urlPriority;
    }

    const urlRemedyType = searchParams.get('remedyType');
    if (urlRemedyType && typeof urlRemedyType === 'string') {
      initialFilters.remedyType = urlRemedyType;
    }

    const urlSourceType = searchParams.get('sourceType');
    if (urlSourceType && typeof urlSourceType === 'string') {
      initialFilters.sourceType = urlSourceType;
    }

    const urlSlaStatus = searchParams.get('slaStatus');
    if (urlSlaStatus && typeof urlSlaStatus === 'string') {
      initialFilters.slaStatus = urlSlaStatus;
    }

    const urlCounterparty = searchParams.get('counterparty');
    if (urlCounterparty && typeof urlCounterparty === 'string') {
      initialFilters.search = urlCounterparty;
    }

    setLocalFilters(initialFilters);
  }, [externalFilters, searchParams]);

  const filteredCases = useMemo(() => {
    const remedyFilters = {};

    if (localFilters.status && typeof localFilters.status === 'string') {
      remedyFilters.status = localFilters.status;
    }

    if (localFilters.priority && typeof localFilters.priority === 'string') {
      remedyFilters.priority = localFilters.priority;
    }

    if (localFilters.remedyType && typeof localFilters.remedyType === 'string') {
      remedyFilters.remedyType = localFilters.remedyType;
    }

    if (localFilters.sourceType && typeof localFilters.sourceType === 'string') {
      remedyFilters.sourceType = localFilters.sourceType;
    }

    if (localFilters.slaStatus && typeof localFilters.slaStatus === 'string') {
      if (localFilters.slaStatus === 'breached') {
        remedyFilters.slaBreached = true;
      } else if (localFilters.slaStatus === 'on_track') {
        remedyFilters.slaBreached = false;
      }
    }

    if (localFilters.search && typeof localFilters.search === 'string') {
      remedyFilters.search = localFilters.search;
    }

    remedyFilters.sortBy = 'createdAt';
    remedyFilters.sortDirection = 'desc';

    return filterRemedies(remedyFilters);
  }, [localFilters, filterRemedies]);
  ...
  const hasActiveFilters =
    ...
    localFilters.remedyType ||
    localFilters.sourceType ||
    localFilters.slaStatus ||
    localFilters.search;
  ...
  const stats = useMemo(() => {
    ...
    for (const remedyCase of filteredCases) {
      if (!remedyCase) continue;

      if (remedyCase.status === 'open') counts.open++;
      else if (remedyCase.status === 'assigned' || remedyCase.status === 'in_progress') counts.inProgress++;
      else if (remedyCase.status === 'pending_counterparty') counts.pendingCounterparty++;
      else if (remedyCase.status === 'escalated') counts.escalated++;
      else if (remedyCase.status === 'resolved' || remedyCase.status === 'closed') counts.resolved++;
...
FilteredRemedyList.defaultProps = {
  filters: null,
};

export default FilteredRemedyList;